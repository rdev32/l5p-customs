...
function Set-ComputerModelName {
  ...
  if (!$model) { 
    Write-Host "No se pudo encontrar el modelo ¿Esta computadora una laptop?"
    return -1
  }
...
# TODO: Not working, the path always exists find another way to check safe mode 
function Test-SafeMode {
  param()

  $registryPath = "HKLM:\SYSTEM\CurrentControlSet\Control\SafeBoot"
  
  if (!(Test-Path $registryPath)) {
    Write-Host "Windows is not currently booted in Safe Mode."
    return -1
  }
  
  Write-Host "Windows is currently booted in Safe Mode."
  return 
}
...
function Install-SoftwarePackage {
  ...
  foreach ($name in $Packages) {
    $currentProgress++
    $progressPercent = ($currentProgress / $Packages.Count) * 100
    $progressStatus = "($($currentProgress) / $($Packages.Count)) $($name)"
    
    if (Invoke-ProcessInstallation -command "winget list --id $($name) -e") {
      Write-Host "El programa $($name) ya esta instalado`t"
      continue
    }
    
    Write-Progress -Activity "Instalando $($name)" -Status $progressStatus -PercentComplete $progressPercent
...
$Host.UI.RawUI.WindowTitle = "Legion 5 Pro Customization Tools v1"
while (1) {
  Clear-Host
  Write-Host @"
